...
const menus = [
	{
		name: __( 'Settings', 'md-ai-content-writer' ),
		path: 'settings',
	},
	{
		name: __( 'Contents', 'md-ai-content-writer' ),
		path: 'contents',
	},
];
		...
		<div className="sticky top-0 md:top-[32px] 600px:top-[46px] right-0 bg-white -ml-5 shadow z-10">
			<div className="relative flex justify-between h-16 max-w-3xl mx-auto px-8 lg:max-w-7xl">
				<div className="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
					<span className="flex-shrink-0 text-3xl flex items-center">
						{__( 'MD AI Content Writer', 'md-ai-content-writer' )}
					</span>
					<div className="sm:ml-8 sm:flex sm:space-x-8">
						{ menus.map( ( menu, key ) => (
							<Link
								...
								to={ {
									pathname: 'admin.php',
									search: `?page=md_ai_content_writer_settings${
										'' !== menu.path
											? '&path=' + menu.path
											: ''
									}`,
				...
				<div className="absolute right-0 flex items-center pr-2 -mr-5 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
					<button
						title={ __( 'Notification', 'md-ai-content-writer' ) }
